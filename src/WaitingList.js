...
function WaitingList() {
  ...
  return (
    <div>
      <h1 class="font-bold text-2xl text-red-700 mt-2 mb-3 text-center">
        Enter WaitingList::
      </h1>
      <div>
        <table class="text-center table-auto w-2/3  mx-auto ">
          <thead>
            <tr class="border-2 border-red-700">
              ...
              <th class="border-2 text-xl border-red-700 text-center">
                MobileNo
              </th>
              <th class="border-2 text-xl border-red-700 text-center">
                No_Of_Person
          ...
          <tbody>
            <tr class="border-2 ">
              <td class="border-2 text-xl border-red-700 text-center">
                <input
        ...
        {showlist && (
          <div>
            <table class="table-auto w-3/4 mx-auto">
              <thead>
                <tr class="border-2">
                  <th class="border-red-700 text-xl border-2 text-center">
                    Sr_No
                  </th>
                  <th class="border-2 text-xl border-red-700 text-center">
                    Name
              ...
              {waitingData.map((val) => {
                return (
                  <tbody>
                    <tr class="border-2">
                      <td class="border-red-700 text-xl border-2 text-center">
                        {val.srno}
                      </td>
                      <td class="border-2 text-xl border-red-700 text-center">
                        {val.name}
                      ...
                      <td class="text-xl border-2 border-red-700 text-center">
                        <select
                          value={val.book}
                          onChange={(e) => TableBookOrNot(e, val.srno)}
                        >
                          <option value="No">No</option>
                          <option value="Yes">Yes</option>
                        </select>
                      </td>
                      <td class="border-2 border-red-700 text-center text-xl">
                        Link
                      </td>
                      <td class="border-red-700 border-2 text-center">
                        <button
                          disabled={val.book == "No"}
                          class={`${
                            val.book == "No"
                              ? "bg-red-700 text-white px-1 py-1 text-center opacity-10 mt-1 mb-1 rounded-md font-bold"
                              : "text-white font-bold  bg-red-700 px-1 py-1 mt-1 mb-1 rounded-md "
                          }`}
                          onClick={() => Proceed(val.srno)}
                        >
                          Proceed
                        </button>
                      </td>
